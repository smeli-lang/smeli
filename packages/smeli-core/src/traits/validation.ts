import { Overload, OverloadedFunction, TypedConstructor, TypedValue } from "../types";
...
export const argumentCount = (count: number) => (traitName: string, overload: Overload) => {
  if (overload.argumentTypes.length !== count) {
    throw new Error(`Trait '${traitName}' always takes ${count} arguments`);
  }
}

export const argumentType = (index: number, type: TypedConstructor<TypedValue>) => (traitName: string, overload: Overload) => {
...
export const returnType = (type: TypedConstructor<TypedValue>) => (traitName: string, overload: Overload) => {
